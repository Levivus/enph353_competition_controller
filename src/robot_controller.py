...
class topic_publisher:
    ...
    def update_clue(self, cv_image):
        ...
        if len(holes) > 0:
            ...
            if clue_size > self.state.max_area:
                ...
                self.state.best_clue = cv2.warpPerspective(cv_image,matrix,(clue.shape[1], clue.shape[0]),flags=cv2.INTER_LINEAR), True
    ...
    def find_words(self):
        ...
        for i in range(len(clue_letter_imgs)):
            ...
            prediction = self.letter_model.predict(letter)
